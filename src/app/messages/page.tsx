...
function MessagesContent() {
  ...
  const fetchConversations = useCallback(async () => {
    try {
      const res = await fetch('/api/messages', { headers: token ? { Authorization: `Bearer ${token}` } : {} });
  ...
  const fetchMessages = useCallback(async (convId: number) => {
    setLoadingMsgs(true);
    try {
      const res = await fetch(`/api/messages?with=${convId}`, { headers: token ? { Authorization: `Bearer ${token}` } : {} });
      const data = await res.json();
  ...
  useEffect(() => {
    fetchConversations();
    // Handle ?with= query param (from listing page "message seller" button)
    const withParam = searchParams.get('with');
    if (withParam) setActiveConvId(parseInt(withParam));
  }, []);
  ...
  const sendMessage = async () => {
    ...
    // Optimistic update
    const optimistic = { id: Date.now(), sender_id: currentUser?.id, content: text, type: 'text', created_at: new Date().toISOString() };
  ...
  const activeConv = conversations.find((c: any) => c.other_user_id === activeConvId) || null;
      ...
      <div style={{ display: 'flex', flex: 1, height: 'calc(100vh - 72px)', marginTop: 72, overflow: 'hidden' }}>
        ...
        <div style={{ width: 320, background: 'white', borderLeft: '1px solid #E2E8F0', display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
          <div style={{ padding: '20px 16px', borderBottom: '1px solid #E2E8F0' }}>
            ...
            <input id="conv-search" value={searchQ} onChange={e => setSearchQ(e.target.value)}
              placeholder="🔍 بحث في المحادثات..."
              style={{ width: '100%', padding: '10px 14px', border: '1.5px solid #E2E8F0', borderRadius: 12, fontFamily: 'Tajawal, sans-serif', fontSize: 13, outline: 'none', background: '#F8FAFC' }} />
          </div>
          <div style={{ flex: 1, overflowY: 'auto' }}>
            {loadingConvs ? (
              Array.from({ length: 4 }).map((_, i) => (
                <div key={i} style={{ margin: '8px 12px', height: 64, borderRadius: 12, background: '#F8FAFC', animation: 'pulse-glow 1.5s infinite' }} />
              ))
            ) : filtered.length === 0 ? (
              ...
              const isActive = activeConvId === conv.other_user_id;
              const initials = (conv.other_display_name || 'م').charAt(0);
              return (
                <div key={conv.other_user_id} id={`conv-${conv.other_user_id}`}
                  onClick={() => setActiveConvId(conv.other_user_id)}
        ...
        {!activeConvId ? (
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={{ fontSize: 14, color: '#94A3B8' }}>أو ابدأ محادثة جديدة من صفحة الإعلان</div>
          </div>
        ) : (
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
            ...
            <div style={{ padding: '14px 24px', background: 'white', borderBottom: '1px solid #E2E8F0', display: 'flex', alignItems: 'center', gap: 14 }}>
              <div style={{ width: 44, height: 44, borderRadius: 14, background: 'linear-gradient(135deg, #1E3A8A, #2563EB)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, color: 'white', fontWeight: 900 }}>
                {(activeConv?.other_display_name || 'م').charAt(0)}
              </div>
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 800, fontSize: 16, color: '#0F172A' }}>
                  {activeConv?.other_display_name || activeConv?.other_username || `مستخدم #${activeConvId}`}
                </div>
              ...
              <button id="report-btn" style={{ width: 38, height: 38, borderRadius: 10, border: '1.5px solid #E2E8F0', background: 'white', cursor: 'pointer', fontSize: 16, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>⚠️</button>
            </div>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '24px', display: 'flex', flexDirection: 'column', gap: 12 }}>
              {loadingMsgs ? (
                ...
                <div style={{ textAlign: 'center', padding: 40, color: '#94A3B8' }}>
                  <div style={{ fontSize: 40, marginBottom: 8 }}>👋</div>
                  <div>ابدأ المحادثة الآن</div>
                </div>
              ) : messages.map((msg: any) => {
                const isMe = msg.sender_id === currentUser?.id || String(msg.sender_id) === String(currentUser?.id);
            ...
            <div style={{ padding: '16px 24px', background: 'white', borderTop: '1px solid #E2E8F0', display: 'flex', gap: 12, alignItems: 'flex-end' }}>
              ...
              />
              <button id="send-btn" onClick={sendMessage} disabled={sending || !input.trim()}
